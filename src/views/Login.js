    ...
    const login = async () => {
        ...
        if(!email){
            ...
            try{
                ...
                setAuth({user: null, loading: true, token: res.data})
             ...
             <Card style={{ width: 450, borderRadius: "20px", background: "#fff", boxShadow: " 0px 8px 15px -2px rgba(0,0,0,0.1)" }}>
                 <div style={{width: "100%", margin:"20px 0px 20px 0px",display: "flex", alignItems: "center", flexDirection: "column"}}>
                     <div style={{width: "78%", display: "flex",alignItems:"center", justifyContent: "space-between"}}>
                        ...
                        <img src={logo} style={{width: "calc(48px * 0.55)", height:"calc(50px * 0.55)"}}></img>
                     </div>
                    ...
                    <Input.Password size="default"autoComplete="off" id="password" placeholder="Enter password" onChange={handleForm} style={{borderRadius: "25px", height:"40px", width: "80% ",margin: "10px 0px 25px 0px"}} prefix={<LockOutlined style={{marginRight: "10px"}}/>} />
